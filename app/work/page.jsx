"use client"

import { motion } from "framer-motion";
import React, { useState } from "react";

import { Swiper, SwiperSlide } from "swiper/react";
import "swiper/css"

import { BsArrowUpRight, BsGithub } from "react-icons/bs"

import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip";

import Link from "next/link";
import Image from "next/image";
import WorkSliderButtons from "@/components/WorkSliderButtons";


const projects = [
  {
    num: "01",
    category: "fullstack",
    description: "Create a game similar to Card Against Humanity board game.",
    stack: [{name: "MERN"}, {name: "Prisma ORM"}, {name: "ChakraUI"}],
    image: "/assets/work/fullstack.jpg",
    github: "https://github.com/vhsxuz/statements-against-mankind"
  },
  {
    num: "02",
    category: "database",
    description: "Create database design about transaction system.",
    stack: [{name: "MSSQL"}],
    image: "/assets/work/database.jpg",
    github: "https://github.com/vhsxuz/database-lab-project"
  },
  {
    num: "03",
    category: "fullstack",
    description: "Create an online clothes store and stock management system.",
    stack: [{name: "PHP"}, {name: "Laravel"}, {name: "Bootstrap"}],
    image: "/assets/work/php.png",
    github: "https://github.com/vhsxuz/maiboutique"
  },
  {
    num: "04",
    category: "iOS Application",
    description: "Create a replica of widgets feature from Blibli iOS mobile application.",
    stack: [{name: "Swift"}, {name: "UIKit"}],
    image: "/assets/work/blibli.png",
    github: "https://github.com/vhsxuz/blibli-widgets"
  },
  {
    num: "05",
    category: "fullstack",
    description: "An educational website that helps developer learn create a secure code.",
    stack: [{name: "MERN"}, {name: "React"}],
    image: "/assets/work/quanc.png",
    github: "https://github.com/vhsxuz/quanc-client"
  },
  {
    num: "06",
    category: "fullstack",
    description: "Create a full stack item management system for BLP recruitment test.",
    stack: [{name: "MERN"}, {name: "JWT Token"}],
    image: "/assets/work/backend.png",
    github: "https://github.com/vhsxuz/blp-test"
  },
]

const Work = () => {

  const [project, setProject] = useState(projects[0]);

  const handleSlideChange = (swiper) => {
    const currentIndex = swiper.activeIndex
    setProject(projects[currentIndex])
  }

  return (
    <motion.div
    initial={{opacity: 0}}
    animate={{
      opacity: 1,
      transition: {delay: 2.4, duration: 0.4, ease: "easeIn"}
    }}
    className="min-h-[80vh] flex flex-col justify-center py-12 xl:px-0 "
    >
      <div className="container mx-auto">
        <div className="flex flex-col xl:flex-row xl:gap-[30px]">
          <div className="w-full xl:w-[50%] xl:h-[460px] flex flex-col xl:justify-between order-2 xl:order-none">
            <div className="flex flex-col gap-[30px]">
              {/* outline num */}
              <div className="text-8xl leading-none font-extrabold text-transparent text-outline">
                {project.num}
              </div>
              {/* category */}
              <h2 className="text-[42px] font-bold leading-none text-white group-hover:text-accent-default transition-all duration-500 capitalize">
                {project.category} Project
              </h2>
              <p className="text-white/60">
                {project.description}
              </p>
              <ul className="flex gap-4">
                {project.stack.map((item, index) => {
                  return (
                    <li key={index} className="text-xl text-accent-default">
                      {item.name}
                      {index !== project.stack.length - 1 && ","}
                    </li>
                  )
                })}
              </ul>
              <div className="border border-white/20"></div>
              <div className="">
                <Link href={project.github}>
                  <TooltipProvider delayDuration={100}>
                    <Tooltip>
                      <TooltipTrigger className="w-[70px] h-[70px] rounded-full bg-white/5 flex justify-center items-center group ">
                        <BsGithub className="text-white hover:text-accent-default text-3xl"/>
                      </TooltipTrigger>
                    </Tooltip>
                  </TooltipProvider>
                </Link>
              </div>
            </div>
          </div>
          <div className="w-full xl:w-[50%]">
            <Swiper 
              spaceBetween={30}
              slidesPerView={1}
              className="xl:h-[520px] mb-12"
              onSlideChange={handleSlideChange}
            >
              {projects.map((project, index) => {
                return(
                  <SwiperSlide key={index}>
                    <div className="h-[460px] relative group flex justify-center items-center bg-pink-50/20">
                      <div className="absolute top-0 bottom-0 w-full h-full bg-black/10 z-10"></div>
                      <div className="relative w-full h-full">
                        <Image src={project.image} fill className="object-cover" alt="" />
                      </div>
                    </div>
                  </SwiperSlide>
                )
              })}
              <WorkSliderButtons 
                containerStyles="flex gap-2 absolute right-0 bottom-[calc(50%_-_22px)] xl:bottom-0 z-20 w-full justify-between xl:w-max xl:justify-none" 
                buttonStyles="bg-accent-default hover:bg-accent-hover text-primary text-[22px] w-[44px] h-[44px] flex justify-center items-center transition-all"
                
              />
            </Swiper>
          </div>
        </div>
      </div>
    </motion.div>
  )
}

export default Work